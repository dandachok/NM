...
using vdouble = std::vector<long double>;
using vvdouble = std::vector<std::vector<long double>>;
...
vdouble SweepMethod (vvdouble mat) {
    ...
    q[0] = mat[0][3] / mat[0][1];
    for (int i = 1; i < n; ++i) {
        long double a = mat[i][0];
        long double b = mat[i][1];
        long double c = mat[i][2];
        long double d = mat[i][3];
        p[i] = (i == n - 1) ? 0 : - c / (b + a*p[i - 1]);
